...
$(function() {
	$("#addHdOperation_cancel").on("click", function() {
...
var insertQueryPama = function(){
		...
		queryParams.icon=$("#searchForm").find("#icon").val();
	...
	$("#addHdOperation_save").on("click", function() {
		$('#saveForm').submit();
		$("#parent_win").window("close");
	});
	...
	$("#tt").datagrid({
		method : "post",
		height : $("#body").height() - $('#search_hdOperation').height() - 5,
		width : $("#body").width(),
 		singleSelect: false,
		selectOnCheck: true,
		checkOnSelect: true,
		...
		onBeforeLoad : function(param) {
			{
				var url = "../hdOperation/getHdOperationsForPage?";
				url += "page=" + param.page;
				url += "&rows=" + param.rows;				
				if (param.sort)
					url += "&sort=" + param.sort;
	...
	$("#save").on("click", function() {
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../hdOperation/insertHdOperation");
		$("#parent_win").window({
			width : 274,
			height : 258,
			title : '新增操作流程'
	...
	if(datas==null||datas.length==0){
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../hdOperation/updateHdOperationById");
	...
	if(datas==null||datas.length==0){
	 ...
	 $.ajax({
		 url: '../hdOperation/deleteHdOperationById?id='+datas[0]['id']+'',
			   type: "get",
			   dataType: "json",
			   success: function(data) {
			   if(data.code){
