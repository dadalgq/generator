...
$(function() {
	$("#addLxDept_cancel").on("click", function() {
...
var insertQueryPama = function(){
		...
		queryParams.pid=$("#searchForm").find("#pid").val();
	...
	$("#addLxDept_save").on("click", function() {
		$('#saveForm').submit();
		$("#parent_win").window("close");
	});
	...
	$("#tt").datagrid({
		method : "post",
		height : $("#body").height() - $('#search_lxDept').height() - 5,
		width : $("#body").width(),
 		singleSelect: false,
		selectOnCheck: true,
		checkOnSelect: true,
		...
		onBeforeLoad : function(param) {
			{
				var url = "../lxDept/getLxDeptsForPage?";
				url += "page=" + param.page;
				url += "&rows=" + param.rows;				
				if (param.sort)
					url += "&sort=" + param.sort;
	...
	$("#save").on("click", function() {
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../lxDept/insertLxDept");
		$("#parent_win").window({
			width : 274,
			height : 225,
			title : '新增部门'
	...
	if(datas==null||datas.length==0){
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../lxDept/updateLxDeptById");
	...
	if(datas==null||datas.length==0){
	 ...
	 $.ajax({
		 url: '../lxDept/deleteLxDeptById?deptId='+datas[0]['deptId']+'',
			   type: "get",
			   dataType: "json",
			   success: function(data) {
			   if(data.code){
