...
$(function() {
	$("#addLxRole_cancel").on("click", function() {
...
var insertQueryPama = function(){
		...
		queryParams.roleName=$("#searchForm").find("#roleName").val();
		queryParams.roleDesc=$("#searchForm").find("#roleDesc").val();
	...
	$("#addLxRole_save").on("click", function() {
		$('#saveForm').submit();
		$("#parent_win").window("close");
	});
	...
	$("#tt").datagrid({
		method : "post",
		height : $("#body").height() - $('#search_lxRole').height() - 5,
		width : $("#body").width(),
 		singleSelect: false,
		selectOnCheck: true,
		checkOnSelect: true,
		...
		onBeforeLoad : function(param) {
			{
				var url = "../lxRole/getLxRolesForPage?";
				url += "page=" + param.page;
				url += "&rows=" + param.rows;				
				if (param.sort)
					url += "&sort=" + param.sort;
		...
		roleId:$("#searchForm").find("#roleId").val()
			,		roleName:$("#searchForm").find("#roleName").val()
			,		roleDesc:$("#searchForm").find("#roleDesc").val()
	...
	$("#save").on("click", function() {
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../lxRole/insertLxRole");
		$("#parent_win").window({
			width : 274,
			height : 192,
			title : '新增角色信息'
	...
	if(datas==null||datas.length==0){
		...
		$('#saveForm')[0].reset();
		$('#saveForm').attr("action","../lxRole/updateLxRoleById");
	...
	if(datas==null||datas.length==0){
	 ...
	 $.ajax({
		 url: '../lxRole/deleteLxRoleById?roleId='+datas[0]['roleId']+'',
			   type: "get",
			   dataType: "json",
			   success: function(data) {
			   if(data.code){
